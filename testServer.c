...
int main(){
    ...
    HANDLE threads[MAX_CLIENTS];    //线程数组
    int CountClient = 0;
    ...
    while(TRUE)
    {
        ...
        {//线程数未超过最大值
            threads[CountClient] = CreateThread(NULL, 0, &recvRequest, &clntSock, 0, NULL);  //创建新线程
        }
        else
        {//线程已满，拒绝服务
            char response[100];
            createResponse(response, "500 Internal Server Error");
            send(clntSock, response, BUF_SIZE, 0);  //返回500 response
            printf("Threads is fully used!\n");
...
//接受并处理request的线程函数
DWORD WINAPI recvRequest(LPVOID lpParam) {
